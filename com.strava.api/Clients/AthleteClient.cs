...
namespace com.Strava.Api.Clients
{
    ...
    public class AthleteClient : BaseClient
    {
        ...
        {
            String getUrl = String.Format("{0}?access_token={1}", new Endpoints().Athlete, Authentication.AccessToken);
        ...
        {
            String getUrl = String.Format("{0}/{1}?access_token={2}", new Endpoints().Athletes, athleteId, Authentication.AccessToken);
            String json = await WebRequest.SendGetAsync(new Uri(getUrl));
        ...
        {
            String getUrl = String.Format("{0}/friends?access_token={1}", new Endpoints().Athlete, Authentication.AccessToken);
            String json = await WebRequest.SendGetAsync(new Uri(getUrl));
        ...
        {
            String getUrl = String.Format("{0}?access_token={1}", new Endpoints().Follower, Authentication.AccessToken);
            String json = await WebRequest.SendGetAsync(new Uri(getUrl));
        ...
        {
            String getUrl = String.Format("{0}/{1}/followers?access_token={2}", new Endpoints().Followers, athleteId, Authentication.AccessToken);
            String json = await WebRequest.SendGetAsync(new Uri(getUrl));
        ...
        {
            String getUrl = String.Format("{0}/{1}/both-following?access_token={2}", new Endpoints().Followers, athleteId, Authentication.AccessToken);
            String json = await WebRequest.SendGetAsync(new Uri(getUrl));
        ...
        public async Task<Athlete> UpdateAthleteAsync(AthleteParameter parameter, String value)
        {
            ...
            switch (parameter)
            {
                case AthleteParameter.City:
                    putUrl = String.Format("{0}?city={1}&access_token={2}", new Endpoints().Athlete, value, Authentication.AccessToken);
                    break;
                case AthleteParameter.Country:
                    putUrl = String.Format("{0}?country={1}&access_token={2}", new Endpoints().Athlete, value, Authentication.AccessToken);
                    break;
                case AthleteParameter.State:
                    putUrl = String.Format("{0}?state={1}&access_token={2}", new Endpoints().Athlete, value, Authentication.AccessToken);
                    break;
                case AthleteParameter.Weight:
                    putUrl = String.Format("{0}?weight={1}&access_token={2}", new Endpoints().Athlete, value, Authentication.AccessToken);
                    break;
            }
        ...
        {
            String putUrl = String.Format("{0}?sex={1}&access_token={2}", new Endpoints().Athlete, gender.ToString().Substring(0, 1), Authentication.AccessToken);
            String json = await WebRequest.SendPutAsync(new Uri(putUrl));
